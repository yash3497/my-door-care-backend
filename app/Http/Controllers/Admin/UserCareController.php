...
class UserCareController extends Controller
{
    ...
    public function userDetails($username)
    {
        ...
        $active_service = UserRequest::where('user_id', $user_id)->where('status', 1)->count();
        $baby_service = UserRequest::where('user_id', $user_id)->where('service_type', 1)->count();
        $pet_service = UserRequest::where('user_id', $user_id)->where('service_type', 2)->count();
    ...
    public function sendMailUsers(Request $request)
    {
        ...
        switch ($request->user_type) {
            case "active";
                $users = User::active()->appDevsEmail()->get();
                break;
            case "all";
                $users = User::get();
                break;
