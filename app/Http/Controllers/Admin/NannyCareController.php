...
use App\Models\NannyMailLog;
use Illuminate\Http\Request;
use App\Models\NannyLoginLog;
...
use App\Notifications\Admin\SendMailNanny;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\Notification;


class NannyCareController extends Controller
{
    /**
    ...
    {
        $page_title = "All Nannies";
        $users = Nanny::orderBy('id', 'desc')->paginate(12);
        return view('admin.sections.nanny-care.index', compact(
            'page_title',
            'users'
        ));
    }
    ...
    {
        $page_title = "Active Nannies";
        $users = Nanny::active()->orderBy('id', 'desc')->paginate(12);
        return view('admin.sections.nanny-care.index', compact(
            'page_title',
            'users'
        ));
    ...
    {
        $page_title = "Banned Nannies";
        $users = Nanny::banned()->orderBy('id', 'desc')->paginate(12);
        return view('admin.sections.nanny-care.index', compact(
            'page_title',
            'users',
        ));
    ...
    public function emailUnverified()
    {
        ...
        $users = Nanny::active()->orderBy('id', 'desc')->emailUnverified()->paginate(12);
        return view('admin.sections.nanny-care.index', compact(
            'page_title',
            'users'
        ));
    ...
    public function KycUnverified()
    {
        ...
        $users = Nanny::kycUnverified()->orderBy('id', 'desc')->paginate(8);
        return view('admin.sections.nanny-care.index', compact(
            'page_title',
            'users'
        ));
    ...
    public function emailAllUsers()
    {
        ...
        return view('admin.sections.nanny-care.email-to-users', compact(
            'page_title',
        ));
    }
    ...
    {
        $page_title = "Nanny Details";
        $user = Nanny::where('username', $username)->first();
        if (!$user) return back()->with(['error' => ['Opps! Nanny not exists']]);
        $nanny_id = $user->id;



        $total_earning = UserRequest::where('nanny_id', $nanny_id)->where('status', 6)->pluck('total')->sum();
        $total_service = UserRequest::where('nanny_id', $nanny_id)->whereIn('status', [1, 4, 5, 6])->count();
        $task_complate = UserRequest::where('nanny_id', $nanny_id)->whereIn('status', [4, 5, 6])->count();
        $incomplete_task = UserRequest::where('nanny_id', $nanny_id)->where('status', 2)->count();


        $data = [
            'total_earning'     => $total_earning,
            'total_service'    => $total_service,
            'task_complate'    => $task_complate,
            'incomplete_task'    => $incomplete_task,
        ];
        return view('admin.sections.nanny-care.details', compact(
            'page_title',
            'user',
            'data',
    ...
    public function sendMailUsers(Request $request)
    {
        ...
        switch ($request->user_type) {
            case "active";
                $users = Nanny::active()->appDevsEmail()->get();
                break;
            case "all";
                $users = Nanny::get();
                break;
            case "email_verified";
                $users = Nanny::emailVerified()->get();
                break;
            case "kyc_verified";
                $users = Nanny::kycVerified()->get();
                break;
            case "banned";
                $users = Nanny::banned()->get();
                break;
        }
    ...
    public function sendMail(Request $request, $username)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'message'       => 'required|string|max:2000',
            'username'      => 'required|string|exists:nannies,username',
        ]);
        if ($validator->fails()) {
        ...
        $user = Nanny::where("username", $username)->first();
        $validated['nanny_id'] = $user->id;
        $validated = Arr::except($validated, ['username']);
        $validated['method']   = "SMTP";
        try {
            NannyMailLog::create($validated);
            $user->notify(new SendMailNanny((object) $validated));
        } catch (Exception $e) {
            return back()->with(['error' => ['Something went worng! Please try again']]);
        }
    ...
    public function userDetailsUpdate(Request $request, $username)
    {
        $request->merge(['username' => $username]);
        $validator = Validator::make($request->all(), [
            'username'              => "required|exists:nannies,username",
            ...
            'zip_code'              => "nullable|max_digits:8",
    ...
    public function loginLogs($username)
    {
        ...
        $logs = NannyLoginLog::where('nanny_id', $user->id)->paginate(12);
        return view('admin.sections.nanny-care.login-logs', compact(
            'logs',
            'page_title',
        ));
    }
    ...
    public function mailLogs($username)
    {
        ...
        $user = Nanny::where("username", $username)->first();
        if (!$user) return back()->with(['error' => ['Opps! User doesn\'t exists']]);
        $logs = NannyMailLog::where("nanny_id", $user->id)->paginate(12);
        return view('admin.sections.nanny-care.mail-logs', compact(
            'page_title',
            'logs',
        ));
    }
    ...
    public function loginAsMember(Request $request, $username)
    {
        ...
        $request->validate([
            'target'            => 'required|string|exists:nannies,username',
            'username'          => 'required_without:target|string|exists:nannies',
        ]);

        try {
            $user = Nanny::where("username", $request->username)->first();
    ...
    public function kycDetails($username)
    {
        ...
        return view('admin.sections.nanny-care.kyc-details', compact("page_title", "user"));
    }
    ...
    public function kycApprove(Request $request, $username)
    {
        $request->merge(['username' => $username]);
        $request->validate([
            'target'        => "required|exists:nannies,username",
            'username'      => "required_without:target|exists:nannies,username",
        ]);
        $user = Nanny::where('username', $request->target)->orWhere('username', $request->username)->first();
    ...
    public function kycReject(Request $request, $username)
    {
        ...
        $user = Nanny::where("username", $request->target)->first();
    ...
    public function search(Request $request)
    {
        ...
        $validated = $validator->validate();
        $users = Nanny::search($validated['text'])->limit(10)->get();
        return view('admin.components.search.nanny-search', compact(
            'users',
        ));
    }
