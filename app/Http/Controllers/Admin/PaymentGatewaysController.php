...
class PaymentGatewaysController extends Controller
{
    ...
    /**
     * Desplay Sell Coin Manual Gateways
     * @return view
     */
    public function manualSellCoinView()
    {
        $page_title = 'Sell Coin';
        $payment_gateways = PaymentGateway::sellCoin()->manual()->get();
        return view('admin.sections.payment-gateways.sell-coin.manual.index', compact('page_title', 'payment_gateways'));
    }
    ...
    /**
     * Display The Edit Page of Sell Coin Manual Gateway
     * @return view
     */
    public function manualSellCoinEdit($alias)
    {

        $page_title = "Receiving Method";
        $payment_gateway = PaymentGateway::sellCoin()->manual()->firstOrFail();

        return view('admin.sections.payment-gateways.sell-coin.manual.edit', compact(
            'page_title',
            'payment_gateway',
        ));
    }
    ...
    /**
     * Function for create add new manual payment gateway page for sell coin
     * @return view
     */
    public function manualSellCoinCreate()
    {
        $page_title = 'Sell Coin Add';
        return view('admin.sections.payment-gateways.sell-coin.manual.create', compact('page_title'));
    }
    ...
    public function manualMoneyOutStore(Request $request)
    {
        ...
        $gateway_name = $request->gateway_name;
        $validator = Validator::make($request->all(), [
            'gateway_name'          => ['required', 'string', 'max:60', Rule::unique('payment_gateways', 'alias')
                ->where(function ($query) use ($gateway_name) {
                    $alias = Str::slug($gateway_name);
    ...
    /**
     * Function for Store New Manual Payment Gateway for Sell Coin
     * @param  \Illuminate\Http\Request  $request
     */
    public function manualSellCoinStore(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'gateway_name'          => ['required', 'string', 'max:60', Rule::unique('payment_gateways', 'alias')
                ->where(function ($query) use ($gateway_name) {
                    $alias = Str::slug($gateway_name);
                    $query->where('slug', PaymentGatewayConst::sell_coin_slug())
                        ->where('type', PaymentGatewayConst::MANUAL)
                        ->where('alias', $alias);
                })],
            'desc'                  => 'nullable|string|max:10000',
        ...
        $validator->after(function ($validator) use ($gateway_name) {
            // Search Gateway is unique or not
            if (PaymentGateway::sellCoin()->manual()->gateway(Str::slug($gateway_name))->exists()) {
                $validator->errors()->add(
                    'gateway_name',
                    'The gateway name has already been taken.'
                );
        ...
        $validated['alias']                 = Str::slug($validated['gateway_name']);
        $validated['name']                  = $validated['gateway_name'];
        $validated['slug']                  = Str::slug(PaymentGatewayConst::SELLCOIN);
        $validated['title']                 = $validated['name'] . " " . "Gateway";
        $validated['type']                  = PaymentGatewayConst::MANUAL;
        $validated['last_edit_by']          = Auth::user()->id;
        $validated['supported_currencies']  = [$validated['currency_code']];
    ...
    /**
     * Function for Update Manual Sell Coin Payment Gateway Information Based On Alias
     * @param  \Illuminate\Http\Request  $request
     * @param string $alias
     */
    public function manualSellCoinUpdate(Request $request, $alias)
    {

        // Find gateway is available or not
        $gateway = PaymentGateway::sellCoin()->manual()->gateway($alias)->first();
        if (!$gateway) {
            return back()->with(['error' => ['Opps! Payment gateway not found!']]);
        }
        // Validate Data
        ...
        $validator = Validator::make($request->all(), [
            'gateway_name'          => ['required', 'string', 'max:60', Rule::unique('payment_gateways', 'alias')
                ->where(function ($query) use ($gateway_name, $gateway) {
                    $alias = Str::slug($gateway_name);
                    $query->whereNot('id', $gateway->id)
                        ->where('slug', PaymentGatewayConst::sell_coin_slug())
                        ->where('type', PaymentGatewayConst::MANUAL)
                        ->where('alias', $alias);
                })],
            'desc'                  => 'nullable|string|max:10000',
