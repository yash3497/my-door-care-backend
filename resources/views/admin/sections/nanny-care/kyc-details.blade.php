...
@section('content')
    ...
    <div class="custom-card mt-15">
        <div class="card-header">
            ...
            @include('admin.components.link.custom', [
                'href' => setRoute('admin.nannies.details', $user->username),
                'text' => 'Profile',
                'class' => 'btn btn--base',
                ...
                <div class="product-sales-btn">
                    @if ($user->kyc_verified != global_const()::VERIFIED)
                        @include('admin.components.button.custom', [
                            ...
                            'text' => 'Approve',
                            'permission' => 'admin.nannies.kyc.approve',
                        ])
                    @endif
                    ...
                    @if ($user->kyc_verified != global_const()::REJECTED)
                        @include('admin.components.button.custom', [
                            ...
                            'text' => 'Reject',
                            'permission' => 'admin.nannies.kyc.reject',
                        ])
                    @endif
                </div>
            @else
    ...
    @include('admin.components.modals.nanny-kyc-reject', compact('user'))
@endsection

@push('script')
    <script>
        $(".approve-btn").click(function() {
            var actionRoute = "{{ setRoute('admin.nannies.kyc.approve', $user->username) }}";
